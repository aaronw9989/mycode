...
def main(): 
    ...
    name = {classinfo.get("all")[0].get("name")}
    spirit_animal = {classinfo.get("all")[0].get("spirit animal")}     
    print(f'{classinfo.get("all")[0].get("name")}')
    print(f'My name is {name} and my spirit and my spirit animal is {spirit_animal}')

    class_list = classinfo.get("all")
    for dic in class_list: 
        print(f'{dic.get("name")} is a {dic.get("skill level")} {dic.get("spirit animal")} of a programmer, posses a {dic.get("super power")} factor for moonlighting as a superhero')
